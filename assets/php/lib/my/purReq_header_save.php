<?php
	class purReq_header_save extends EditTable {
		...
		public function idno($purreqno,$recno){
			$sql="SELECT idno FROM material.purreqhd WHERE purreqno = '$purreqno' AND recno = '$recno'";
			...
			try{
				if($commit){
					...
					if($this->oper == 'add'){
						...
						$purreqno = $this->request_no('PR',$_POST['purreqhd_reqdept']);
						$recno = $this->recno('PUR','PR');

						$addarrField=['purreqno','recno','compcode','adduser','adddate','upduser','upddate','recstatus','reqpersonid'];//extra field
						$addarrValue=[$purreqno,$recno,$this->compcode,$this->user,'NOW()',$this->user,'NOW()','OPEN',$this->user];
						...
						$responce->idno = $this->idno($purreqno,$recno);
