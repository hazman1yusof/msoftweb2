<?php
	class EditTableCM{
		...
		public function toGetAllapacthdr($auditno){
			$query = "SELECT bankcode, cheqno FROM finance.apacthdr WHERE auditno ='$auditno' AND compcode='{$_SESSION['company']}' AND source= 'CM' AND trantype=?";
			//echo $query;
			$result = $this->db->query($query);if (!$result) { print_r($this->db->errorInfo()); }
			$resultarr = $result->fetch(PDO::FETCH_ASSOC);
			return $resultarr;
	...
	/*	public function toGetAllchqtran($auditno){
			$query = "SELECT bankcode, cheqno FROM finance.chqtran WHERE auditno ='$auditno' AND compcode='{$_SESSION['company']}' AND source= 'CM' AND trantype=?";
			//echo $query;
			$result = $this->db->query($query);if (!$result) { print_r($this->db->errorInfo()); }
			$resultarr = $result->fetch(PDO::FETCH_ASSOC);
			return $resultarr;
