			...
			try{
				...
				//1st step -> 2nd phases, update bankdtl
					if($this->isCBtranExist($seldata['bankcode'],$tempobj->year,$tempobj->period)){
						...
						$arrayValue = array($this->cbtranAmount-$seldata['amount'],$seldata['bankcode'],$tempobj->year);
				...
				//2nd step 2nd phases, update bankdtl
					if($this->isCBtranExist($seldata['payto'],$tempobj->year,$tempobj->period)){

						// $totamt = $this->getCbtranTotamt($seldata['payto'],$tempobj->year,$tempobj->period);

						$prepare = "UPDATE finance.bankdtl SET actamount".$tempobj->period." = ? WHERE bankcode =? AND year =?";
						$arrayValue = array($this->cbtranAmount+$seldata['amount'],$seldata['payto'],$tempobj->year);
