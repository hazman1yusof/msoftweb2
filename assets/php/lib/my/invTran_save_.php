<?php
	class invTran{
		...
		private function sysparam2($txndept, $trantype){
			$sysparam2 = $this->sysparam2;

			$sqlSysparam="SELECT seqno FROM material.sequence WHERE  dept = '$txndept' AND trantype = '$trantype'";
			$result = $this->db->query($sqlSysparam);if (!$result) { print_r($this->db->errorInfo()); }
			$row = $result->fetch(PDO::FETCH_ASSOC);

			$seqno=intval($row['seqno'])+1;

			$sqlSysparam="UPDATE material.sequence SET seqno = '$seqno' WHERE dept = '$txndept' AND trantype = '$trantype'";
		...
		public function toGetAllapacthdr($auditno){
			$query = "SELECT bankcode, cheqno FROM finance.apacthdr WHERE auditno ='$auditno' AND compcode='{$_SESSION['company']}' AND source= 'CM' AND trantype='DP'";
			//echo $query;
			$result = $this->db->query($query);if (!$result) { print_r($this->db->errorInfo()); }
			$resultarr = $result->fetch(PDO::FETCH_ASSOC);
			return $resultarr;
		...
		public function toGetAllchqtran($auditno){
			$query = "SELECT bankcode, cheqno FROM finance.chqtran WHERE auditno ='$auditno' AND compcode='{$_SESSION['company']}' AND source= 'CM' AND trantype='DP'";
			//echo $query;
			$result = $this->db->query($query);if (!$result) { print_r($this->db->errorInfo()); }
			$resultarr = $result->fetch(PDO::FETCH_ASSOC);
			return $resultarr;
				...
				if($this->oper=='add'){
					...
					if(!empty($this->sysparam)){
						...
						if($this->oper=='add' && $this->returnVal){
							$this->responce->recno=$tempsysparam;
							//$this->responce->sndrcv=$sndrcv;
							//$this->responce->trandate=$trandate;
						}
					}

					if(!empty($this->sysparam2)){
						$tempsysparam = $this->sysparam2($txndept, $trantype);
						array_push($addarrField,$this->sysparam2['useOn']);
						array_push($addarrValue,$tempsysparam);

						if($this->oper=='add' && $this->returnVal){
							$this->responce->docno=$tempsysparam;
						}
					}
