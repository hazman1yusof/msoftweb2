				...
				//		b- ivtmpdt --> ivtxndt
						...
						while($row = $result->fetch(PDO::FETCH_ASSOC)) {
							...
							if($this->stockExpExist($seldata['txndept'],$itemcode,$uomcode,$expdate,$batchno)){
								...
								$prepare = "UPDATE material.stockexp SET balqty = '$txnqty' + '$bal' WHERE compcode=? AND deptcode = ? AND itemcode = ? AND uomcode =? AND expdate=? AND batchno=?";
							...
							if($this->stocklocExist($cy,$itemcode,$uomcode,$seldata['txndept'])){
								...
								$prepare = "UPDATE material.stockloc SET qtyonhand = '$txnqty' + '$oldqtyonhand', netmvqty$trandateMonth = '$oldnetqty' + '{$qtyonhand}', netmvval$trandateMonth = $oldnetval + '{$amount}' WHERE compcode=? AND year=? AND itemcode = ? AND uomcode =?  AND deptcode = ?";
							...
							//4)Product
							$prepare = "UPDATE material.product SET qtyonhand = '$txnqty' + '$oldqtyonhand' WHERE compcode=? AND itemcode = ? AND uomcode =?";
