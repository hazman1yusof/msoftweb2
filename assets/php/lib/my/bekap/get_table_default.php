<?php
	class TableModel{
		...
		///////Where use !=////////
		var $filterNotEqlCol;
		var $filterNotEqlType; //NOT IN or IN
		var $filterNotEqlVal;
	    ...
	    {	
			...
			if(!empty($get['filterNotEqlCol'])){
				$this->filterNotEqlCol=$get['filterNotEqlCol'];
				$this->filterNotEqlType=$get['filterNotEqlType'];
				$this->filterNotEqlVal=$get['filterNotEqlVal'];
			}
		...
		private function filterNotEql($first){
			$addSql='';
			if(!isset($this->filterNotEqlCol) || empty($this->filterNotEqlCol)){
				return $addSql;
			}else{
				foreach ($this->filterNotEqlCol as $key => $col) {
					$type=$this->filterNotEqlType[$key];
					$val=$this->filterNotEqlVal[$key];
					$addSql.="AND {$col} {$type} '{$val}' ";
					$addSql=rtrim($addSql,',');
				}
				if($first){
					return strstr($addSql,' ');
		...
		private function autoPrepStmt($needSearch,$needFilter,$countOnly,$forTable){
			...
			if(is_array($this->table)){
				foreach($this->table as $index => $value){
					if($index==0){
						...
						if(!empty($this->join_filterCol)){
							foreach ($this->join_filterCol as $key => $col) {
								$val=$this->join_filterVal[$key];
								if(strpos($val, '.') !== false){ /// kalu ada .(dot)
									$pieces = explode(".", $val, 2);
									if($pieces[0] == 'skip'){
			...
			if(!empty($this->filterNotEqlCol)){
				$string.=$this->filterNotEql($first);	
			}
