	...
	class dpreg_save{
		...
		public function edit_table($commit){
			try{
				...
				//1st step add cbtran -
					$prepare = "INSERT INTO finance.cbtran (compcode,bankcode,source,trantype,auditno,postdate,year,period,cheqno,amount,remarks,upduser,upddate,bitype,reference,stat,refsrc,reftrantype,refauditno) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?,?,?,?,?)";

					$arrayValue = array($_SESSION['company'],$seldata['bankcode'],$seldata['source'],$seldata['trantype'],$seldata['auditno'],$seldata['actdate'],$tempobj->year,$tempobj->period,$seldata['cheqno'],-$seldata['amount'],$seldata['remarks'],$_SESSION['username'],null,'Payto :'. ' ' .$seldata['payto']  . ' ' . $seldata[ 'remarks'],'A',null,null,null);
				...
				//2nd step step add gltran
					...
					while($row = $result->fetch(PDO::FETCH_ASSOC)) {
						...
						$arrayValue = array($_SESSION['company'],$auditno, $lineno_.'1',$source,$trantype,$cheqno,$remarks,$tempobj->year,$tempobj->period,$debitbank['costcode'],$creditbank['glccode'],$debitbank2['expacct'],$creditbank['glaccno'],$amountb4gst,null,$actdate,$_SESSION['username']);
						...
						if($this->isGltranExist($debitbank['costcode'],$debitbank2['expacct'],$tempobj->year,$tempobj->period)){
							...
							$arrayValue = array($this->gltranAmount+$amountb4gst,$debitbank['costcode'],$debitbank2['expacct'],$tempobj->year);
							...
							$arrayValue = array($_SESSION['company'],$debitbank['costcode'],$debitbank2['expacct'],$tempobj->year,$amountb4gst,$_SESSION['username'],'A');
						...
						$arrayValue = array($_SESSION['company'],$auditno, $lineno_.'2',$source,$trantype,$cheqno,$remarks,$tempobj->year,$tempobj->period,$gst['pvalue1'],$creditbank['glccode'],$gst['pvalue2'],$creditbank['glaccno'],$amountgst,null,$actdate,$_SESSION['username']);
