		...
		public function edit_table($commit){
			...
			try{
				...
				//1st step add cbtran -
					...
					$arrayValue = array($_SESSION['company'],$seldata['bankcode'],$seldata['source'],$seldata['trantype'],$seldata['auditno'],$seldata['actdate'],$tempobj->year,$tempobj->period,$seldata['cheqno'],-$seldata['amount'],$seldata['remarks'],$_SESSION['username'],null,$seldata['payto'],'A',null,null,null);
				...
				//2nd step step add gltran
					...
					$queryDP = "SELECT d.compcode, d.source, d.trantype, d.auditno, d.lineno_, d.document, h.remarks, 
									d.deptcode, d.category, h.bankcode, d.amount, h.actdate
					...
					while($row = $result->fetch(PDO::FETCH_ASSOC)) {
						...
						$arrayValue = array($_SESSION['company'],$auditno, $lineno_,$source,$trantype,$document,$remarks,$tempobj->year,$tempobj->period,$debitbank['costcode'],$creditbank['glccode'],$debitbank2['expacct'],$creditbank['glaccno'],$amount,null,$actdate,$_SESSION['username']);
				...
				//3th step add glmasdtl untuk bankcode
					...
					if($this->isPaytypeCheque($seldata['paymode']) == 'Cheque') {
						$prepare = "UPDATE finance.chqtran SET cheqdate=?, amount=?, remarks=?, stat = ?, lastuser = ?, lastupdate = NOW(), trantype =?, source = ?, auditno = ? 
